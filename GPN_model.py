...
class GPNConv(torch.nn.Module):
    def __init__(self, nn, input_dim):
        super(GPNConv, self).__init__()
        self.nn = nn

        self.transformerBlock = TransformerBlock(input_size=input_dim)
    def forward(self, x, edge_index, weight):
        ...
        out = self.transformerBlock(out, out, out)
...
class GPN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, input_dim, hidden_dim, output_dim):
        ...
        for layer in range(num_layers - 1):
            ...
            gpn_conv = GPNConv(mlp, hidden_dim)
